...
export interface AzureOpenAIEntraSettings {
...
export class AzureOpenAIEntraHandler extends BaseProvider {
	...
	private options: AzureOpenAIEntraSettings

	constructor(options: AzureOpenAIEntraSettings) {
		super()
		this.options = options

		// Set up model info - use openAiCustomModelInfo if available, otherwise use defaults
		const modelId = options.azureOpenAiModelId || options.azureOpenAiDeploymentName || "gpt-4"
		this.model = {
			id: modelId,
			info: {
	...
	async *createMessage(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): ApiStream {
		...
		const stream = await client.chat.completions.create({
			...
			temperature: 0, // Default temperature, can be made configurable
		})
